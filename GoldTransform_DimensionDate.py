...
# MAGIC %md
# MAGIC # GOLD Transform Dimension Date
...
df_source = spark.sql('''
    select Distinct(Date_ID) as DateID from parquet.`abfss://silver@cardemodatalake.dfs.core.windows.net/CarSalesData`
''')
...
df_filter = df_source.join(df_sink, df_source['DateID'] == df_sink['DateID'], how='left').select(df_source['DateID'], df_sink['dim_Date_Key'])
...
if Load_Flag == '0':
    ...
    df_max_value = spark.sql("Select max('dim_Date_key') from cars_catalog.gold.dim_date")
...
if spark.catalog.tableExists('cars_catalog.gold.dim_date'):
    # Incremental Data
    delta_tbl = DeltaTable.forPath(spark, "abfss://gold@cardemodatalake.dfs.core.windows.net/dim_date")
    ...
    df_final.write.format("delta").mode("overwrite").option("path","abfss://gold@cardemodatalake.dfs.core.windows.net/dim_date").saveAsTable("cars_catalog.gold.dim_date")
