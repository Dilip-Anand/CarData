...
# MAGIC %md
# MAGIC # GOLD Transform Dimension Dealer
...
df_source = spark.sql('''
    select Distinct(Dealer_ID) as DealerID, DealerName as DealerNAME from parquet.`abfss://silver@cardemodatalake.dfs.core.windows.net/CarSalesData`
''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer') :
    ...
    df_sink = spark.sql('''
    select dim_Dealer_Key, DealerID, DealerName 
...
df_filter = df_source.join(df_sink, df_source['DealerID'] == df_sink['DealerID'], how='left').select(df_source['DealerID'], df_source['DealerNAME'], df_sink['dim_Dealer_Key'])
...
df_final = df_final.withColumn('dim_Dealer_Key', col('dim_Dealer_Key').cast("BIGINT"))
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer') :
    #incremental Data
    delta_table = DeltaTable.forPath(spark,"abfss://gold@cardemodatalake.dfs.core.windows.net/dim_dealer")
    ...
    df_final.write.format("delta").mode("overwrite").option("path","abfss://gold@cardemodatalake.dfs.core.windows.net/dim_dealer").saveAsTable("cars_catalog.gold.dim_dealer")
